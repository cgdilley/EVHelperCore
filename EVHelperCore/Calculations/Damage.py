...
def calculate_damage(attacker: Pokemon, defender: Pokemon, move: DamagingMove, critical: bool = False) -> Iterable[int]:
    offense_stat = get_stat_value_from_info(attacker.stats, move.offense_stat)
    defense_stat = get_stat_value_from_info(defender.stats, move.defense_stat)

    offense_stat *= attacker.stats.get_modifier(move.offense_stat, minimum=0 if critical else None).multiplier
    defense_stat *= defender.stats.get_modifier(move.defense_stat, maximum=0 if critical else None).multiplier

    damage_ratio = offense_stat / defense_stat
