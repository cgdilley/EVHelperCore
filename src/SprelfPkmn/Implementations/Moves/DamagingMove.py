...
class DamagingMove(Move):
    ...
    def build(cls, name: str,
              ...
              damage_type: DamageClass | None = None,
              accuracy: int | None = None,
              description: str = "", max_pp: int | None = None,
              properties: MoveProperties = MoveProperties.NONE) -> Self:
        if defense_stat is None:
            defense_stat = Stat.DEFENSE if offense_stat == Stat.ATTACK else Stat.SP_DEFENSE
        if damage_type is None:
            damage_type = DamageClass.PHYSICAL if offense_stat in (Stat.ATTACK, Stat.DEFENSE) else DamageClass.SPECIAL
        return cls(name=name, type=typ, base_power=base_power, offense_stat=offense_stat,
    ...
    def get_damage_conditions(self, attacker: PokemonState,
    ...
    def get_stat_conditions(self, attacker: PokemonState,
                            defender: PokemonState,
                            board_state: BoardState) -> Iterable[StatCondition]:
        yield from AbilityModifiers.stat_conditions(move=self,
                                                    attacker=attacker,
                                                    defender=defender,
                                                    board_state=board_state)
    ...
    def get_base_power_modifiers(self, attacker: PokemonState,
                                 defender: PokemonState,
    ...
    def get_stab_modifiers(self, attacker: PokemonState,
                           defender: PokemonState,
                           board_state: BoardState) -> Iterable[STABCondition]:
        if self.type in attacker.typing:
...
class WeatherModifiers:
    ...
    def stat_conditions(cls,
        ...
        if len(board_state.weather_negations) == 0:
            if board_state.weather == Weather.SNOW and Type.ICE in defender.typing \
                    and move.defense_stat == Stat.DEFENSE:
                yield WeatherStatCondition(weather=board_state.weather,
                                           stat=Stat.DEFENSE,
                                           multiplier=1.5,
                                           for_attacker=False)
            if board_state.weather == Weather.SAND and Type.ROCK in defender.typing \
                    and move.defense_stat == Stat.SP_DEFENSE:
                yield WeatherStatCondition(weather=board_state.weather,
...
class AbilityModifiers:
    ...
    def damage_conditions(cls,
        ...
        if defender.ability.name == "Punk Rock" and MoveProperties.SOUND in move.properties:
            yield AbilityDamageCondition(ability=defender.ability, multiplier=0.5)
        if defender.ability.name == "Ice Scales" and move.damage_class == DamageClass.SPECIAL:
            yield AbilityDamageCondition(ability=defender.ability, multiplier=0.5)
    ...
    def stat_conditions(cls, move: DamagingMove,
                        ...
                        defender: PokemonState,
                        board_state: BoardState) -> Iterable[StatCondition]:
        if attacker.ability.name == "Guts" and attacker.status != StatusCondition.NONE:
            yield AbilityStatCondition(ability=attacker.ability, multiplier=1.5, stat=Stat.ATTACK,
                                       for_attacker=True)
        if defender.ability.name == "Marvel Scale" and defender.status != StatusCondition.NONE:
            yield AbilityStatCondition(ability=defender.ability, multiplier=1.5, stat=Stat.DEFENSE,
                                       for_attacker=False)
...
class BoardStateModifiers:
    ...
    def damage_conditions(cls, move: DamagingMove,
        ...
        if attacker.ability.name != "Infiltrator" and not critical:
            ...
            if move.damage_class == DamageClass.PHYSICAL and len(physical_screens) > 0:
                yield ScreensDamageCondition(effect=physical_screens[0].type,
                                             multiplier=TWO_THIRDS if board_state.is_doubles else 0.5)
            elif move.damage_class == DamageClass.SPECIAL and len(special_screens) > 0:
    ...
    @classmethod
    def _is_grounded(cls, pok: PokemonState):
        return Type.FLYING not in pok.typing and pok.ability.name != "Levitate"
...
class ItemModifiers:
    ...
    def base_power_conditions(cls, move: DamagingMove,
        ...
        if attacker.item:
            if attacker.item.name == "Muscle Band" and move.damage_class == DamageClass.PHYSICAL:
                yield ItemBasePowerCondition(item=attacker.item, multiplier=ONE_POINT_ONE)
            if attacker.item.name == "Wise Glasses" and move.damage_class == DamageClass.SPECIAL:
                yield ItemBasePowerCondition(item=attacker.item, multiplier=ONE_POINT_ONE)
    ...
    def damage_conditions(cls, move: DamagingMove,
        ...
        if attacker.item:
            ...
            if attacker.item.name == "Choice Band" and move.damage_class == DamageClass.PHYSICAL:
                yield ItemDamageCondition(item=attacker.item, multiplier=1.5, for_attacker=True)
            if attacker.item.name == "Choice Specs" and move.damage_class == DamageClass.SPECIAL:
                yield ItemDamageCondition(item=attacker.item, multiplier=1.5, for_attacker=True)
            if attacker.item.name == "Expert Belt" and Type.get_damage_multiplier(move.type, defender.typing) > 1:
        ...
        if defender.item:
            if defender.item.name == cls._type_resist_berry(move.type) \
                    ...
                    and (move.type == Type.NORMAL
                         or Type.get_damage_multiplier(move.type, defender.typing) > 1):
